<template>
	<view class="container" v-if="show" :style="{
		 width: getWidth,
		 height: getHeight,
		...
		<view class="marquee-hori" v-if="type==='hori-connect'" @tap="showMoreExtendTap" :style="{
			...
			<view class="box-hori" :style="{
				...
				<text id="text" :style="{
					 ...
					 lineHeight: getheight}">{{text.length<=1 ? text[0] : text.join(join)}}</text>
...
<script>
	export default {
		...
		computed: {
			getWidth() {
				if (/(%|px|rpx|auto)$/.test(this.width)) return this.width;
				else return this.width + 'rpx';
				// return parseFloat() / uni.getSystemInfoSync().screenWidth * 750
			},
			getHeight() {
				if (/(%|px|rpx|auto)$/.test(this.height)) return this.height;
				else return this.height + 'rpx';
			},
			getFontSize(){
				if (/(px|rpx|em|rem)$/.test(this.fontSize)) return this.fontSize;
